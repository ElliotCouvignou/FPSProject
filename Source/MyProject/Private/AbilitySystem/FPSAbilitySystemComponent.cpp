...
UFPSAbilitySystemComponent* UFPSAbilitySystemComponent::GetAbilitySystemComponentFromActor(const AActor* Actor,
                                                            ...
                                                            bool EndAbilityImmediately)
{
	...
	if (InAbilityHandle.IsValid())
	{
		...
		if (EndAbilityImmediately)
		{
			...
			if (AbilitySpec)
			{
				UFPSWeaponGameplayAbility* GSAbility = Cast<UFPSWeaponGameplayAbility>(AbilitySpec->GetPrimaryInstance());
	...
	if (AnimInstance && NewAnimMontage)
	{
		...
		if (Duration > 0.f)
		{
			...
			else
			{
				...
				{
					PredictionKey.NewRejectedDelegate().BindUObject(this, &UFPSAbilitySystemComponent::OnPredictiveMontageRejectedForMesh, InMesh, NewAnimMontage);
				}
			}
		}
	}
...
void UFPSAbilitySystemComponent::InitAbilityActorInfo(AActor* InOwnerActor, AActor* InAvatarActor)
{
	...
	AbilityActorInfo->OwnerActor = InOwnerActor;
}

float UFPSAbilitySystemComponent::PlayMontageSimulatedForMesh(USkeletalMeshComponent* InMesh, UAnimMontage* NewAnimMontage, float InPlayRate, FName StartSectionName)
{
	float Duration = -1.f;
	UAnimInstance* AnimInstance = IsValid(InMesh) && InMesh->GetOwner() == AbilityActorInfo->AvatarActor ? InMesh->GetAnimInstance() : nullptr;
	if (AnimInstance && NewAnimMontage)
...
FGameplayAbilityRepAnimMontageForMesh& UFPSAbilitySystemComponent::GetGameplayAbilityRepAnimMontageForMesh(
	USkeletalMeshComponent* InMesh)
{
...
void UFPSAbilitySystemComponent::OnPredictiveMontageRejectedForMesh(USkeletalMeshComponent* InMesh,
...
void UFPSAbilitySystemComponent::AnimMontage_UpdateReplicatedDataForMesh(
	...
	for (FGameplayAbilityRepAnimMontageForMesh& NewRepMontageInfoForMesh : RepAnimMontageInfoForMeshes)
	{
		...
		if (!AbilityActorInfo->IsLocallyControlled())
		{
			...
			if (NewRepMontageInfoForMesh.RepMontageInfo.AnimMontage)
			{
				...
				if ((AnimMontageInfo.LocalMontageInfo.AnimMontage != NewRepMontageInfoForMesh.RepMontageInfo.AnimMontage) || (AnimMontageInfo.LocalMontageInfo.PlayInstanceId != static_cast<uint8>(ReplicatedPlayBit)))
				{
