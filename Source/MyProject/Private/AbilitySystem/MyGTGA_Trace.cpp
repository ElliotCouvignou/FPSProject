...
void AMyGTGA_Trace::ResetSpread()
{
	bUseAimingSpreadMod = false;
	BaseSpread = 0.0f;
	AimingSpreadMod = 0.0f;
...
float AMyGTGA_Trace::GetCurrentSpread() const
{
...
void AMyGTGA_Trace::SetStartLocation(const FGameplayAbilityTargetingLocationInfo& InStartLocation)
{
	StartLocation = InStartLocation;
}

void AMyGTGA_Trace::SetShouldProduceTargetDataOnServer(bool bInShouldProduceTargetDataOnServer)
{
	ShouldProduceTargetDataOnServer = bInShouldProduceTargetDataOnServer;
}

void AMyGTGA_Trace::SetDestroyOnConfirmation(bool bInDestroyOnConfirmation)
{
	bDestroyOnConfirmation = bDestroyOnConfirmation;
}

void AMyGTGA_Trace::StartTargeting(UGameplayAbility* Ability)
{
...
void AMyGTGA_Trace::ConfirmTargetingAndContinue()
{
...
void AMyGTGA_Trace::CancelTargeting()
{
...
void AMyGTGA_Trace::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
	DestroyReticleActors();
...
void AMyGTGA_Trace::Tick(float DeltaSeconds)
{
...
void AMyGTGA_Trace::LineTraceWithFilter(TArray<FHitResult>& OutHitResults, const UWorld* World, const FGameplayTargetDataFilterHandle FilterHandle, const FVector& Start, const FVector& End, FName ProfileName, const FCollisionQueryParams Params)
{
	check(World);
...
void AMyGTGA_Trace::AimWithPlayerController(const AActor* InSourceActor, FCollisionQueryParams Params, const FVector& TraceStart, FVector& OutTraceEnd, bool bIgnorePitch)
{
...
void AMyGTGA_Trace::StopTargeting()
{
	SetActorTickEnabled(false);
...
FGameplayAbilityTargetDataHandle AMyGTGA_Trace::MakeTargetData(const TArray<FHitResult>& HitResults) const
{
...
TArray<FHitResult> AMyGTGA_Trace::PerformTrace(AActor* InSourceActor)
{
...
AGameplayAbilityWorldReticle* AMyGTGA_Trace::SpawnReticleActor(FVector Location, FRotator Rotation)
{
	if (ReticleClass)
	{
...
void AMyGTGA_Trace::DestroyReticleActors()
{
