...
#include "AbilitySystem/Tasks/PlayMontageForMeshAndWaitEvent.h"
...
UPlayMontageForMeshAndWaitEvent::UPlayMontageForMeshAndWaitEvent(const FObjectInitializer& ObjectInitializer)
...
UFPSAbilitySystemComponent* UPlayMontageForMeshAndWaitEvent::GetTargetASC()
{
	return Cast<UFPSAbilitySystemComponent>(AbilitySystemComponent);
}

void UPlayMontageForMeshAndWaitEvent::OnMontageBlendingOut(UAnimMontage* Montage, bool bInterrupted)
{
...
void UPlayMontageForMeshAndWaitEvent::OnAbilityCancelled()
{
...
void UPlayMontageForMeshAndWaitEvent::OnMontageEnded(UAnimMontage* Montage, bool bInterrupted)
{
	if (!bInterrupted)
	{
		if (ShouldBroadcastAbilityTaskDelegates())
...
void UPlayMontageForMeshAndWaitEvent::OnGameplayEvent(FGameplayTag EventTag, const FGameplayEventData* Payload)
{
	if (ShouldBroadcastAbilityTaskDelegates())
	{
...
UPlayMontageForMeshAndWaitEvent* UPlayMontageForMeshAndWaitEvent::PlayMontageForMeshAndWaitForEvent(UGameplayAbility* OwningAbility,
	...
	float OverrideBlendOutTimeForStopWhenEndAbility)
{
	...
	UPlayMontageForMeshAndWaitEvent* MyObj = NewAbilityTask<UPlayMontageForMeshAndWaitEvent>(OwningAbility, TaskInstanceName);
	MyObj->Mesh = InMesh;
...
void UPlayMontageForMeshAndWaitEvent::Activate()
{
	if (Ability == nullptr)
	{
		return;
	...
	if (GSAbilitySystemComponent)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = GSAbilitySystemComponent->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UPlayMontageForMeshAndWaitEvent::OnGameplayEvent));
			...
			if (GSAbilitySystemComponent->PlayMontageForMesh(Ability, Mesh, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection, bReplicateMontage) > 0.f)
			{
				...
				CancelledHandle = Ability->OnGameplayAbilityCancelled.AddUObject(this, &UPlayMontageForMeshAndWaitEvent::OnAbilityCancelled);

				BlendingOutDelegate.BindUObject(this, &UPlayMontageForMeshAndWaitEvent::OnMontageBlendingOut);
				AnimInstance->Montage_SetBlendingOutDelegate(BlendingOutDelegate, MontageToPlay);

				MontageEndedDelegate.BindUObject(this, &UPlayMontageForMeshAndWaitEvent::OnMontageEnded);
...
void UPlayMontageForMeshAndWaitEvent::ExternalCancel()
{
...
void UPlayMontageForMeshAndWaitEvent::OnDestroy(bool AbilityEnded)
{
...
bool UPlayMontageForMeshAndWaitEvent::StopPlayingMontage(float OverrideBlendOutTime)
{
	if (Mesh == nullptr)
	{
		return false;
...
FString UPlayMontageForMeshAndWaitEvent::GetDebugString() const
{
